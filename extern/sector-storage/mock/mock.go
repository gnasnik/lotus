...
type mockVerifProver struct {
	aggregates map[string]minertypes.AggregateSealVerifyProofAndInfos // used for logging bad verifies
}
...
func (mgr *SectorMgr) GenerateWinningPoSt(ctx context.Context, minerID abi.ActorID, xSectorInfo []minertypes.ExtendedSectorInfo, randomness abi.PoStRandomness) ([]minertypes.PoStProof, error) {
	mgr.lk.Lock()
	defer mgr.lk.Unlock()

	sectorInfo := make([]minertypes.SectorInfo, len(xSectorInfo))
	for i, xssi := range xSectorInfo {
		sectorInfo[i] = minertypes.SectorInfo{
...
func (mgr *SectorMgr) GenerateWindowPoSt(ctx context.Context, minerID abi.ActorID, xSectorInfo []minertypes.ExtendedSectorInfo, randomness abi.PoStRandomness) ([]minertypes.PoStProof, []abi.SectorID, error) {
	mgr.lk.Lock()
	defer mgr.lk.Unlock()
	...
	si := make([]minertypes.ExtendedSectorInfo, 0, len(xSectorInfo))
	...
	sectorInfo := make([]minertypes.SectorInfo, len(si))
	for i, xssi := range si {
		sectorInfo[i] = minertypes.SectorInfo{
			SealProof:    xssi.SealProof,
			SectorNumber: xssi.SectorNumber,
...
func generateFakePoStProof(sectorInfo []minertypes.SectorInfo, randomness abi.PoStRandomness) []byte {
	randomness[31] &= 0x3f
...
func generateFakePoSt(sectorInfo []minertypes.SectorInfo, rpt func(abi.RegisteredSealProof) (abi.RegisteredPoStProof, error), randomness abi.PoStRandomness) []minertypes.PoStProof {
...
func (mgr *SectorMgr) GenerateWinningPoStWithVanilla(ctx context.Context, proofType abi.RegisteredPoStProof, minerID abi.ActorID, randomness abi.PoStRandomness, proofs [][]byte) ([]minertypes.PoStProof, error) {
	panic("implement me")
}

func (mgr *SectorMgr) GenerateWindowPoStWithVanilla(ctx context.Context, proofType abi.RegisteredPoStProof, minerID abi.ActorID, randomness abi.PoStRandomness, proofs [][]byte, partitionIdx int) (minertypes.PoStProof, error) {
	panic("implement me")
}
...
func (m mockVerifProver) VerifySeal(svi minertypes.SealVerifyInfo) (bool, error) {
...
func (m mockVerifProver) VerifyAggregateSeals(aggregate minertypes.AggregateSealVerifyProofAndInfos) (bool, error) {
...
func (m mockVerifProver) VerifyReplicaUpdate(update minertypes.ReplicaUpdateInfo) (bool, error) {
	return true, nil
}

func (m mockVerifProver) AggregateSealProofs(aggregateInfo minertypes.AggregateSealVerifyProofAndInfos, proofs [][]byte) ([]byte, error) {
...
func (m mockVerifProver) VerifyWinningPoSt(ctx context.Context, info minertypes.WinningPoStVerifyInfo) (bool, error) {
	info.Randomness[31] &= 0x3f
	return true, nil
}

func (m mockVerifProver) VerifyWindowPoSt(ctx context.Context, info minertypes.WindowPoStVerifyInfo) (bool, error) {
...
var MockVerifier = mockVerifProver{
	aggregates: map[string]minertypes.AggregateSealVerifyProofAndInfos{},
}
